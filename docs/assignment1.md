...
| F14 | Graphical User Interface | Graphical User Interface is a variation of a terminal-based User Interface. The GUI we are aiming to implement is interactive design for multiple features, such as inventory, movement directions buttons, movement possibility highlighted. The inventory shall be presented as a set of empty slots. The slot will be filled after the character will pick up an item. The slot with an item that can be used in the room shall be highlighted. The slot will become empty after using an item. When it comes to movement, there shall be four clickable buttons pointing to North, East, South and West shall be presented on the screen. Button(-s) with possible movement directions from this room shall be highlighted. | The Graphical User Interface feature aims to make a game experience more straightforward and enjoyable for a player. The GUI shall be implemented in order to increase user-friendliness, simplify the process of learning for a player and decrease the chance of making an input mistake by a player. |
